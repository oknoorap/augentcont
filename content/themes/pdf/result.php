...
foreach(results() as $item)
{
	...
	$the_ref[]		= $item['url'];
	$the_links[]	= $item['id'];
	$new_links[]    = generate_permalink($item['title'], get_category());
}
...
foreach($the_content as $index => $value)
{
	...
	$the_link = array('id' => $the_links[$index]);
	$content .= "<p>{$value} <a href=\"". permalink($the_link) ."\">Continue Reading</a>, <a href=\"". $new_links[$index] ."\">Similiar</a> <a href=\"#3\"><sup>{$content_count}</sup></a></p>";
}
