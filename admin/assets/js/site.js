...
.controller('Main', ['$scope', '$rootScope', '$timeout', function (s, r, t) {
	...
	s.boostMode = window.boostMode;
	s.theme = window.theme;
	s.selectTheme = function (name) {
		s.theme = name;
	};
	s.permalinks = (typeof permalinks !== "undefined")? permalinks: {};
	s.showPassword = {password: false};
	s.databases = (typeof databases !== "undefined")? databases: {};
	s.databases.status = false;
	s.buildPermalink = function (str) {
		if (s.permalinks.capitalize) {
			str = str.toLowerCase().replace(/\b[a-z]/g, function(letter) {
				return letter.toUpperCase();
			})
		}
		str = str.replace(/\s/g, s.permalinks.separator);
		return str;
	};
				...
				columns: [
					...
					{ field: 'status', caption: '', size: '30px', attr: 'align="center"', render: function (record) {
						...
						if (status === true) {
							...
							return '<i class="fa fa-remove" title="Failed"></i>';
						}
						else {
	...
	s.prepareKeywords = function () {
		var keywords = r.keywords.split("\n").map(function (item) {
			...
			item = S($.trim(uslug(S(item).humanize().underscore().latinise().s.replace(/[\_\/]/g, ' ')))).humanize().s;
			return item;
		...
		if (keywords.length > 0) {
			...
			t(function () {
				...
				$.each(keywords, function (index, item) {
					if(_.where(w2ui.table.records, {keyword: item}).length === 0 && item.split(/\s+/).length < 13) {
						list.push({
							keyword: item,
							status: '',
			...
			var q = async.queue(function (task, callback) {
				$.ajax({
					...
					data: {
						...
						icon: r.selectedIcon,
						keyword: task.keyword.toLowerCase(),
						index: task.index,
						type: 'insert'
					},
					dataType: 'JSON',
